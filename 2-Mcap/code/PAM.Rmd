...
```{r}
PAM_exp <- PAM #%>% filter(timepoint!="Acclimation" & timepoint!="Recovery")
table(PAM_exp$plug)

ggplot(PAM_exp,aes(x = date, y = fv_fm_y_1000)) + 
    geom_boxplot(aes(fill = treatment)) +theme_minimal() +scale_fill_manual(values = custom_colors) 

ggplot(PAM_exp,aes(x = date, y = fv_fm_y_1000, group = plug)) + 
