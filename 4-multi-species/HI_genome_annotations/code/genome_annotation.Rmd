...
```{r}
Mcap_SwissP <- read.delim("../annotation/Montipora_capitata_HIv3_Swissprot_GO.tsv") %>% dplyr::rename(GOs = GeneOntologyIDs)
...
#write.csv(for_natalie_added, file = "../annotation/NC_HeatStress_Genes.csv",row.names = FALSE, quote = FALSE)

Natalie_list <- read.csv(file = "../annotation/NC_HeatStress_Genes.csv")
```
...
```{r}
Bhattacharya2016 <- read.csv("../references/calcium_membrane_transport/2_Gene_list7.csv",header = TRUE) %>%
...
for (sp in species){
  bltabl <- read.csv(paste0("../references/calcium_membrane_transport/",sp,"_channels_BLAST_out.tab"), sep = '\t', header = FALSE)
...
for (i in 1:length(species)){
  ...
  write_csv(channel_df,paste0("../annotation/",species[i],"_membrane_channels.csv"))
}
```
