---
title: "RNA-seq-QC"
output: html_document
---

```{r}
library(tidyverse)
```


```{r}
# load in multiqc data
QC_raw_full <- read.delim("../output_RNA/raw_qc/multiqc_data_1/multiqc_fastqc.txt", header = TRUE)
QC_raw <- read.delim("../output_RNA/raw_qc/multiqc_data_1/multiqc_general_stats.txt", header = TRUE)

colnames(QC_raw) <- str_replace(colnames(QC_raw) , "FastQC_mqc.generalstats.fastqc.", "") 
```

```{r}
QC_raw <- QC_raw %>%
  mutate(sample_id = str_replace(Sample, "run-2-", "")) %>%
  mutate(sample_id = str_replace(sample_id, "run-3_", "")) %>%
  mutate(sample_id = str_replace(sample_id, "_S[0-9]+", ""))

QC_raw_summarized <- QC_raw %>%
  group_by(sample_id) %>%
  summarise(
    total_seq = sum(total_sequences),
    n_fastqs = n(),
    .groups = "drop"
  ) %>%
  mutate(total_Mseq = total_seq/1000000)

QC_raw_summarized %>% arrange(total_Mseq) %>% filter(total_Mseq < 15) 
paste(nrow(QC_raw_summarized %>% arrange(total_Mseq) %>% filter(total_Mseq < 15))/2, "samples have fewer than 15M reads in each direction")

write.csv(QC_raw_summarized %>% arrange(total_Mseq), "../output_RNA/raw_qc/multiqc_data/concatenated_runs_nreads.csv")
```


```{r}
# load in multiqc data
QC_trim <- read.delim("../output_RNA/trimmed_qc/multiqc_data_1/multiqc_fastqc.txt", header = TRUE)
#QC_trim <- read.delim("../output_RNA/trimmed_qc/multiqc_data/multiqc_general_stats.txt", header = TRUE)
```

```{r}
QC_trim <- QC_trim %>%
  mutate(sample_id = str_replace(Sample, "run-2-", "")) %>%
  mutate(sample_id = str_replace(sample_id, "run-3_", "")) %>%
  mutate(sample_id = str_replace(sample_id, "_S[0-9]+", ""))

QC_trim_summarized <- QC_trim %>%
  group_by(sample_id) %>%
  summarise(
    total_seq = sum(Total.Sequences),
    n_fastqs = n(),
    .groups = "drop"
  ) %>%
  mutate(total_Mseq = total_seq/1000000)

QC_trim_summarized %>% arrange(total_Mseq) %>% filter(total_Mseq < 15) 
paste(nrow(QC_trim_summarized %>% arrange(total_Mseq) %>% filter(total_Mseq < 15))/2, "samples have fewer than 15M reads in each direction")

write.csv(QC_trim_summarized %>% arrange(total_Mseq) , "../output_RNA/trimmed_qc/multiqc_data/concatenated_runs_nreads_post_trim.csv")
```

### mapping rates

```{r}
POR_map <- read.delim("../output_RNA/alignment_qc/POR_Pcomp/multiqc_data/multiqc_star.txt", header = TRUE) %>% mutate(species="POR")
MON_map <- read.delim("../output_RNA/alignment_qc/MON_MCapV3/multiqc_data/multiqc_star.txt", header = TRUE) %>% mutate(species="MON")
POC_map <- read.delim("../output_RNA/alignment_qc/POC_PacutaV2/multiqc_data/multiqc_star.txt", header = TRUE) %>% mutate(species="POC")

mapping_all <- bind_rows(POC_map,MON_map,POR_map)
```

```{r}
for (sp in c("POC","MON","POR")){
  df <- mapping_all %>% filter(species ==sp)
  
  # IQR method 
  Q1 <- quantile(df$uniquely_mapped_percent, 0.25)
  Q3 <- quantile(df$uniquely_mapped_percent, 0.75)
  IQR_val <- Q3 - Q1
  
  lower_bound <- Q1 - 1.5 * IQR_val
  upper_bound <- Q3 + 1.5 * IQR_val
  outliers <- df[df$uniquely_mapped_percent < lower_bound | df$uniquely_mapped_percent > upper_bound, ]
  print(paste0(sp,": IQR = ", Q1,"-",Q3,"; 1.5 x IQR = ",lower_bound,"-",upper_bound,"; ",nrow(outliers)," outliers"))
  print(outliers)
}
```


### kraken contamination and mapping comparison

```{r}
kraken <- read.csv("../output_RNA/contam_screen/contamination_kraken.csv") %>% mutate(species = substr(sample,1,3))

kraken %>% ggplot(aes(x=sample,y = percent_reads,fill=classification)) +geom_bar(stat="identity") + facet_grid(~species,scales="free_x") + theme(axis.text.x = element_text(angle=45)) + theme_classic() + scale_fill_brewer(palette = "Set2")
```

```{r}
for (sp in c("POC","MON","POR")){
  df <- kraken %>% filter(species ==sp & classification=="Bacteria")
  
  # IQR method 
  Q1 <- quantile(df$percent_reads, 0.25)
  Q3 <- quantile(df$percent_reads, 0.75)
  IQR_val <- Q3 - Q1
  
  lower_bound <- Q1 - 1.5 * IQR_val
  upper_bound <- Q3 + 1.5 * IQR_val
  outliers <- df[df$percent_reads < lower_bound | df$percent_reads > upper_bound, ]
  print(paste0(sp,": IQR = ", Q1,"-",Q3,"; 1.5 x IQR = ",lower_bound,"-",upper_bound,"; ",nrow(outliers)," outliers"))
  print(outliers)
}
```


```{r}
kraken_map <- kraken %>% 
  filter(classification=="Bacteria") %>%
  left_join(mapping_all %>% select(Sample,total_reads, uniquely_mapped, uniquely_mapped_percent), by = join_by(sample==Sample))

ggplot(kraken_map) + 
  geom_point(aes(x = percent_reads, y =uniquely_mapped_percent, color=classification)) +
  theme_bw() + facet_grid(~species)#,scales="free_x")

```



### rRNA contamination and mapping comparison


```{r}
# load in multiqc data of mapping data
POR_map <- mapping_all %>% filter(species == "POR")
POR_rRNA <- read.csv("../output_RNA/rRNA_screen/rRNA_contamination_bbduk_POR.csv") %>% select(-total_reads)
POR_rRNA_Fede <- read.csv("../output_RNA/rRNA_screen/rRNA_contamination_bbduk_POR_Federica.csv") %>% select(-total_reads)
```

```{r}
POR_rRNA <- POR_rRNA %>% left_join(POR_map %>% select(Sample,total_reads, uniquely_mapped, uniquely_mapped_percent), by = join_by(sample==Sample))

POR_rRNA <- POR_rRNA %>% mutate(total_reads_Million = total_reads/1000000) %>%
  mutate(matched_reads_pairs = matched_reads/2) %>%
  mutate(matched_reads_pairs_Million = matched_reads_pairs/1000000) %>%
  mutate(reads_non_rRNA_Million = total_reads_Million-matched_reads_pairs_Million) %>%
  mutate(uniquely_mapped_Million = uniquely_mapped/1000000)

library(ggplot2)

plot <- ggplot(POR_rRNA) + 
  geom_point(aes(x = percent_rrna, y =uniquely_mapped_percent, color=total_reads_Million)) + theme_bw() + 
  labs(title = "Percent rRNA reads vs. Unique Mapping Rates:\nPOR Samples")
plot
ggsave("images/POR_rRNA_v_mapping.png")

ggplot(POR_rRNA) + 
  geom_point(aes(x = percent_rrna, y =uniquely_mapped_percent, color=total_reads_Million)) + theme_bw() + 
  labs(title = "Percent rRNA reads vs. Unique Mapping Rates:\nPOR Samples")

ggplot(POR_rRNA) + 
  geom_point(aes(x = percent_rrna, y =total_reads_Million, color=total_reads_Million)) + theme_bw() + 
  labs(title = "Percent rRNA reads vs. Unique Mapping Rates:\nPOR Samples")

ggplot(POR_rRNA) + 
  geom_point(aes(x = total_reads_Million, y =uniquely_mapped_percent, color=total_reads_Million)) + theme_bw() + 
  labs(title = "Percent rRNA reads vs. Unique Mapping Rates:\nPOR Samples")

POR_rRNA_summary <- POR_rRNA %>% select(sample,uniquely_mapped_percent,matched_reads_pairs_Million,total_reads_Million,percent_rrna) %>% 
  dplyr::rename(rRNA_matched_reads_pairs_Million=matched_reads_pairs_Million,
                total_reads_pairs_Million = total_reads_Million
                ) %>% arrange(uniquely_mapped_percent) %>%
  mutate(across(contains("_"), ~ round(as.numeric(.x), 2)))


POR_rRNA_summary %>% pivot_longer(cols = contains("_"),
               names_to = "metric",
               values_to = "value") %>%
  ggplot(aes(x = value)) +
  geom_density(fill="lightgray") +
  facet_wrap(~ metric, scales = "free") +
  theme_bw() 


write_csv(POR_rRNA_summary, "../output_RNA/differential_expression/POR_Pcomp/rRNA_and_mapping_metadata.csv")
```

### POR mRNA-decontaminated rarefaction analysis


```{r}
# Directory containing all rarefaction CSV outputs
resdir <- "/scratch4/workspace/zdellaert_uri_edu-shared_TimeSeries/TimeSeries/rRNA_rarefaction/"

# Load all CSVs
csv_files <- list.files(resdir, pattern = "_rarefaction.csv$", recursive = TRUE, full.names = TRUE)

all_results <- map_df(csv_files, function(f) {
  # Extract sample name from parent directory
  sample_name <- basename(dirname(f))
  
  read_csv(f) %>%
    mutate(sample = sample_name)
})

head(all_results)
```

```{r}
all_results <- all_results %>%
  mutate(
    depth = as.numeric(depth),
    pairs_processed = as.numeric(pairs_processed),
    pairs_mapped = as.numeric(pairs_mapped),
    percent_mapped = as.numeric(percent_mapped),
    percent_unique = as.numeric(percent_unique),
    genes_detected = as.numeric(genes_detected),
  )
```

```{r}
# Genes detected
ggplot(all_results, aes(x = depth, y = pairs_processed, color = sample)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Reads processed at each subsampling depth",
    x = "Fraction of original reads used",
    y = "Reads processed"
  ) +
  theme_bw()

# Genes detected
ggplot(all_results, aes(x = depth, y = pairs_mapped, color = sample)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Reads pseudoaligned by subsampling depth",
    x = "Fraction of original reads used",
    y = "Reads pseudoaligned"
  ) +
  theme_bw()

# Genes detected
ggplot(all_results, aes(x = depth, y = genes_detected, color = sample)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Rarefaction of detected genes by subsampling depth",
    x = "Fraction of original reads used",
    y = "Number of genes detected (TPM > 0.5)"
  ) +
  theme_bw()

# Mapping rate
ggplot(all_results, aes(x = depth, y = percent_mapped, color = sample)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(
    title = "Rarefaction of pseudoalignment (mapping) rate by subsampling depth",
    x = "Fraction of original reads used",
    y = "Pseudoalignment (mapping) rate"
  ) +
  theme_bw()


# Mapping rate
ggplot(all_results, aes(x = depth, y = percent_unique, color = sample)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(
    title = "Rarefaction of unique pseudoalignment (mapping) rate by subsampling depth",
    x = "Fraction of original reads used",
    y = "Unique Pseudoalignment (mapping) rate"
  ) +
  theme_bw()

```
```{r}
plateau <- all_results %>%
  group_by(sample) %>%
  arrange(depth) %>%
  mutate(delta = genes_detected - lag(genes_detected))# %>%
  #filter(!is.na(delta) & delta < 50)  # threshold for minimal new genes

head(plateau)
```

```{r}
POR_combined <- all_results %>% left_join(POR_rRNA_summary)

# Genes detected
ggplot(POR_combined, aes(x = depth, y = genes_detected, group=sample, color = uniquely_mapped_percent)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_color_distiller(palette = "Spectral") +
  labs(
    title = "Rarefaction of detected genes by subsampling depth",
    x = "Fraction of original reads used",
    y = "Number of genes detected (TPM > 0.5)"
  ) +
  theme_bw()
```



```{r}
# Load and prepare metadata
metadata <- read.csv("../data_RNA/completed_bulk_RNA_extractions_3species.csv") %>%
  select(fragment_name, t_hours, treatment, Tank, Extraction_Date, 
         RNA_QBIT_AVG, RNA_Gel_quality_score, ng_RNA_total) %>%
  mutate(across(c(t_hours, treatment, Tank, Extraction_Date), factor),
         across(c(RNA_QBIT_AVG, RNA_Gel_quality_score, ng_RNA_total), as.numeric))

# Join with rRNA data
POR_rRNA_summary_meta <- POR_rRNA_summary %>% 
  left_join(metadata, by = join_by("sample" == "fragment_name"))

# Visualize potential batch effects
library(patchwork)

# Continuous variables
cont_plots <- c("RNA_QBIT_AVG", "RNA_Gel_quality_score", "ng_RNA_total") %>%
  map(~ ggplot(POR_rRNA_summary_meta, aes(x = .data[[.x]], y = percent_rrna)) +
        geom_point(alpha = 0.6) +
        geom_smooth(method = "lm", se = TRUE) +
        theme_bw() +
        labs(title = .x))

# Categorical variables (biological + batch)
cat_plots <- c("treatment", "t_hours", "Tank", "Extraction_Date") %>%
  map(~ ggplot(POR_rRNA_summary_meta, aes(x = .data[[.x]], y = percent_rrna, color=treatment)) +
        geom_boxplot(outlier.shape = NA) +
        geom_jitter(alpha = 0.5, width = 0.15) +
        theme_bw() +
        labs(title = .x))

wrap_plots(c(cont_plots, cat_plots))

# Model with treatment×time interaction
model <- lm(percent_rrna ~ RNA_QBIT_AVG + RNA_Gel_quality_score + ng_RNA_total + 
              treatment * t_hours + Tank + Extraction_Date,
            data = POR_rRNA_summary_meta)

summary(model)
anova(model)

# Diagnostic plots
par(mfrow = c(2, 2))
plot(model)
par(mfrow = c(1, 1))

# Examine batch effect (Extraction_Date)
POR_rRNA_summary_meta %>%
  group_by(Extraction_Date) %>%
  summarise(n = n(), 
            mean_rRNA = mean(percent_rrna, na.rm = TRUE),
            sd_rRNA = sd(percent_rrna, na.rm = TRUE)) %>%
  arrange(desc(mean_rRNA))
```



```{r}
POR_rRNA <- read.csv("../output_RNA/rRNA_screen/rRNA_contamination_bbduk_POR.csv")  %>% 
  mutate(total_reads_M_correct = total_reads/1000000)
POR_rRNA_Fede <- read.csv("../output_RNA/rRNA_screen/rRNA_contamination_bbduk_POR_Federica.csv")  %>% 
  mutate(total_reads_M_correct = total_reads/1000000)

POR_rRNA$dataset <- "TimeSeries"
POR_rRNA_Fede$dataset <- "TPC"
combined <- rbind(POR_rRNA,POR_rRNA_Fede)

ggplot(combined, aes(x=percent_rrna, fill = dataset)) +
         geom_histogram(alpha=0.5, binwidth=1) + 
        coord_cartesian(xlim = c(0, 100)) + theme_bw()

library(cowplot) 
pmain <- ggplot(combined, aes(x=percent_rrna,y=total_reads_M_correct, color = dataset))+
  geom_point()

xdens <- axis_canvas(pmain, axis = "x")+
  geom_density(data = combined, aes(x = percent_rrna, fill = dataset), alpha = 0.7, size=0.2)

ydens <- axis_canvas(pmain, axis = "y", coord_flip = TRUE)+
  geom_density(data = combined, aes(x = total_reads_M_correct, fill = dataset), alpha = 0.7, size=0.2)+
  coord_flip()
p1 <- insert_xaxis_grob(pmain, xdens, grid::unit(.2, "null"), position = "top")
p2 <- insert_yaxis_grob(p1, ydens, grid::unit(.2, "null"), position = "right")
ggdraw(p2)
```



