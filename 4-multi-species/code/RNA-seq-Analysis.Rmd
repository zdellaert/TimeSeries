...
membrane_channels_unique <- membrane_channels %>% group_by(query) %>%
  summarize(ProteinNames = paste(unique(ProteinNames), collapse = ","),
            gene_set = paste(unique(gene_set), collapse = ","),
            ) %>% select(gene_set, query, ProteinNames) %>%
  mutate(short_name = str_replace(ProteinNames, "\\s+\\(.*", ""),
         short_name = str_replace(short_name, "\\s+\\(.*", ""))

membrane_channels_unique <- membrane_channels_unique %>% filter(query %in% rownames(vsd_mat))
...
membrane_list <- membrane_channels  %>%
  group_by(gene_set) %>%
  summarise(query = list(query), .groups = "drop") %>%
  deframe()

for (genelist in names(membrane_list)){
  genes <- membrane_list[[genelist]]
  
  plot <- plot_df %>% filter(query %in% genes) %>% filter(is_DE == FALSE) %>% 
    ggplot(aes(x=time, y=expression, color=treatment, group=treatment)) +
  stat_summary(fun="mean", geom="line") +
  scale_color_manual(values = treat_colors) +
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.2) +
  facet_wrap(paste0(str_replace(query,"Pocillopora_acuta_HIv2___",""), ": ", genelist)~ ~paste0("DE by LRT: ",is_DE),scales="free_y") +
...
```{r}
channel_clustered <- membrane_channels_unique  %>% unique() %>% 
  left_join(cluster_assignments, by = join_by(query==gene)) %>%
  arrange(cluster)  %>%
  filter(!is.na(cluster)) 

# plot this to show which genes are in which cluster
channel_clustered %>% filter(!is.na(cluster)) %>% ggplot(aes(y=reorder(gene_set, cluster), x=factor(cluster), fill=cluster)) +
  geom_tile() +
  theme_bw() +
  labs(x="Mfuzz Cluster", y="Gene ID", title="Membrane channel genes clustered by temporal expression pattern")

cat("Total membrane channel genes:", nrow(membrane_channels_unique), "\n")
cat("Found in DE/Mfuzz data:", nrow(channel_clustered), "\n")
...
expr_data <- exprs(heat_eset_membrane) %>%
  as.data.frame() %>%
  tibble::rownames_to_column("query") %>%
  pivot_longer(-query, names_to = "timepoint", values_to = "expression") %>%
  mutate(time = as.numeric(factor(timepoint, 
                                  levels = c("R0", "R1", "R3", "R12", "R24", "R72", "R120"))) - 1,
         time_hours = c(0, 1, 3, 12, 24, 72, 120)[time + 1])

# Join with cluster and gene_set info
plot_data <- expr_data %>%
  left_join(channel_clustered %>% select(query, cluster, gene_set, membership,short_name), 
            by = "query")

# Plot by cluster, colored by gene_set
p <- plot_data %>%
  ggplot(aes(x = time_hours, y = expression, group = query, color = gene_set)) +
  geom_line(aes(alpha = membership), linewidth = 0.8) +
  facet_wrap(~cluster, ncol = 3, 
             labeller = labeller(cluster = function(x) paste("Cluster", x))) +
  scale_alpha_continuous(range = c(0.3, 1)) +
  scale_x_continuous(breaks = c(0, 1, 3, 12, 24, 72, 120)) +
  theme_bw() +
  labs(x = "Time (hours)", 
       y = "Normalized Expression",
       color = "Channel Type",
       alpha = "Membership",
       title = "Membrane Channel Gene Expression Patterns by Cluster")

print(p)

gene_trajectories <- plot_data %>%
  group_by(query, cluster, gene_set, membership,short_name) %>%
  summarize(
    max_expr = max(expression),
    max_time = time_hours[which.max(expression)],
    .groups = "drop"
  ) %>%
  group_by(cluster) %>%
  slice_max(membership, n = 3)  # Top 3 genes per cluster
...
p2 <- plot_data %>%
  ggplot(aes(x = time_hours, y = expression, group = query, color = gene_set)) +
  geom_line(aes(alpha = membership), linewidth = 0.8) +
  geom_text_repel(data = gene_trajectories,
                  aes(x = max_time, y = max_expr, label = short_name),
  ...
  labs(x = "Time (hours)", 
       ...
       color = "Channel Type",
       alpha = "Membership",
       title = "Membrane Channel Gene Expression Patterns")
...
for (genelist in names(membrane_list)){
  ...
  scale_color_manual(values = treat_colors) +
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.2) +
  facet_wrap(paste0(str_replace(query,"Montipora_capitata_HIv3___",""), ": ", genelist)~ ~paste0("DE by LRT: ",is_DE),scales="free_y") +
  theme_bw() +
  labs(y="VST expression", x="Timepoint")
  print(plot)
}
...
for (genelist in names(membrane_list)){
  ...
  scale_color_manual(values = treat_colors) +
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.2) +
  facet_wrap(paste0(str_replace(query,"Porites_compressa_HIv1___",""), ": ", genelist)~ ~paste0("DE by LRT: ",is_DE),scales="free_y") +
  theme_bw() +
  labs(y="VST expression", x="Timepoint")
  print(plot)
}
