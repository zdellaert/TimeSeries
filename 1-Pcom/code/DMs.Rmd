---
title: "Daily measurement plotting, Pcomp"
author: "Zoe Dellaert "
date: "6/24/2025"
output: 
  github_document:
...
```{r}
apex <- read_xml("../data/LoggerData/Apex_log.xml")
```
...
meta <- list(
  hostname = xml_text(xml_find_first(apex, ".//hostname")),
  serial = xml_text(xml_find_first(apex, ".//serial")),
  timezone = xml_text(xml_find_first(apex, ".//timezone")),
  software = xml_attr(apex, "software"),
  hardware = xml_attr(apex, "hardware")
)

# Get all <record> nodes
records <- xml_find_all(apex, ".//record")

# Parse each <record> and its <probe> children
all_data <- map_df(records, function(rec) {
  date <- xml_text(xml_find_first(rec, "date"))
  probes <- xml_find_all(rec, "probe")

  map_df(probes, function(probe) {
    tibble(
      date = date,
      name = xml_text(xml_find_first(probe, "name")),
      type = xml_text(xml_find_first(probe, "type")),
      value = as.numeric(xml_text(xml_find_first(probe, "value")))
    )
  })
})

# Add metadata (broadcasted across all rows)
final_df <- bind_cols(all_data, as_tibble(meta))
```
